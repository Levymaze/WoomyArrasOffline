...
if (tdmBaseMode) {
    ...
    modes.push(["growthtdm", growthTdmConfig]);
        ...
        // Growth 2TDM: thin, horizontal team bases with multiple protectors and fixed bot count
        const growth2TdmConfig = applyGrowthDefaults(cloneModeConfig(tdmBaseMode));
        growth2TdmConfig.serverName = "Growth 2TDM";
        growth2TdmConfig.MODE = "tdm";
        growth2TdmConfig.TEAM_AMOUNT = 2;
        // 30 bots total (15 per team approx.)
        growth2TdmConfig.BOTS = 30;
        // Vertical thin bases on left/right as single continuous strips with 4 protectors each
        // Protector rows are centered and spaced (rows 2,4,6,8). Central area has polygon spawners.
        // Layout: continuous bas1 column at x=0 and bas2 at x=11.
        // Add 3-wide platform protrusions from the base at rows 2,5,8,10 (0-based)
        // Protectors (n_b1/n_b2) sit at the outer edge of each platform (one per platform)
        growth2TdmConfig.ROOM_SETUP = [
             ["n_b1","norm","norm","norm","norm","norm","norm","norm","norm","norm","norm","n_b2"],
        ...
        growth2TdmConfig.X_GRID = 12;
        growth2TdmConfig.Y_GRID = 12;
        modes.push(["growth2tdm", growth2TdmConfig]);

    // Growth Solo vs Team: player is solo on BLUE (team 1) against full opposing team (GREEN/team 2)
    const growthSoloVsTeamConfig = applyGrowthDefaults(cloneModeConfig(tdmBaseMode));
    growthSoloVsTeamConfig.serverName = "Growth Solo vs Team";
    growthSoloVsTeamConfig.MODE = "tdm";
    growthSoloVsTeamConfig.TEAM_AMOUNT = 2;
    // 30 bots total, all should spawn on the opposing team side
    growthSoloVsTeamConfig.BOTS = 30;
    // Mark this mode so server bot logic will spawn bots only on enemy side
    growthSoloVsTeamConfig.SOLO_VS_TEAM = true;
    // Reuse the same thin vertical base layout as growth2tdm
    growthSoloVsTeamConfig.ROOM_SETUP = JSON.parse(JSON.stringify(growth2TdmConfig.ROOM_SETUP));
    growthSoloVsTeamConfig.X_GRID = growth2TdmConfig.X_GRID;
    growthSoloVsTeamConfig.Y_GRID = growth2TdmConfig.Y_GRID;
    modes.push(["growthsolovteam", growthSoloVsTeamConfig]);

    // Growth Solo vs Team - Corners: same as growthsolovteam but team bases are small corner bases
    const growthSoloVsTeamCornersConfig = applyGrowthDefaults(cloneModeConfig(tdmBaseMode));
    growthSoloVsTeamCornersConfig.serverName = "Growth Solo vs Team (Corners)";
    growthSoloVsTeamCornersConfig.MODE = "tdm";
    growthSoloVsTeamCornersConfig.TEAM_AMOUNT = 2;
    growthSoloVsTeamCornersConfig.BOTS = 30;
    growthSoloVsTeamCornersConfig.SOLO_VS_TEAM = true;
    // Small corner bases on opposite corners (top-left = blue, bottom-right = green)
    growthSoloVsTeamCornersConfig.ROOM_SETUP = [
        ["n_b1","n_b1","norm","norm","norm","norm","norm","norm","norm","norm","norm","norm"],
        ["n_b1","bas1","norm","norm","norm","norm","norm","norm","norm","norm","norm","norm"],
        ...
        ["norm","norm","norm","norm","norm","norm","norm","norm","norm","norm","norm","norm"],
        ["norm","norm","norm","norm","norm","norm","norm","norm","norm","norm","norm","norm"],
        ["norm","norm","norm","norm","norm","norm","norm","norm","norm","norm","bas2","n_b2"],
        ["norm","norm","norm","norm","norm","norm","norm","norm","norm","norm","n_b2","n_b2"],

    ];
    growthSoloVsTeamCornersConfig.X_GRID = growthSoloVsTeamCornersConfig.ROOM_SETUP[0].length;
    growthSoloVsTeamCornersConfig.Y_GRID = growthSoloVsTeamCornersConfig.ROOM_SETUP.length;
    modes.push(["growthsolovteamcorners", growthSoloVsTeamCornersConfig]);
